...
export function useRevertLocalCommit({ projectId, repository, onSuccess }: UseRevertLocalCommitOptions) {
  ...
  const revertLatestLocalCommit = useCallback(async () => {
    ...
    try {
      const response = await authenticatedFetch('/api/git/revert-local-commit', {
        ...
        body: JSON.stringify({ project: projectId, ...(repository ? { repository } : {}) }),
      });
  ...
  }, [onSuccess, projectId, repository]);
