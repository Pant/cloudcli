...
import { configureDynamicResponseCaching, setStaticResourceCacheHeaders } from '@/shared/utils.js';

async function exerciseLayout(layout: 'source' | 'package'): Promise<void> {
  ...
  await Promise.all([
    ...
    ...(layout === 'source' ? [fs.writeFile(path.join(publicRoot, 'sw.js'), 'source-worker')] : []),
    ...(layout === 'package' ? [fs.writeFile(path.join(distRoot, 'sw.js'), 'packaged-worker')] : []),
  ]);
  ...
  try {
    const [worker, manifest, icon, html, asset, api, version, prefixedVersion] = await Promise.all([
      fetch(`${base}/sw.js`),
    ...
    await Promise.all([worker.text(), manifest.text(), icon.text(), html.text(), asset.text(), api.text(), prefixedVersion.text()]);
  } finally {
    server.close();
...
test('package-like static layout keeps the emitted worker mutable', () => exerciseLayout('package'));
